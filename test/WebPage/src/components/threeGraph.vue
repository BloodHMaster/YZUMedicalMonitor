<template>
  <div>
    ...
    <!-- 绘制图例 -->
    <div id="indicator">
      <!-- 利用item 遍历一�?数组 利用index 遍历另�?�一�?数组 -->
    ...
    <!-- 绘制右边显示结果 -->
    <div v-show="selectNodeData.name !== undefined" id="info">
...
export default {
  ...
  props: {
    ...
    // �?定义图例（数组保证一一对应�?
    // names		图例名称变量制作图标
    // labels		节点的标签名称（与records.json�?保证相同�?
    names: {
      type: Array
    },
    labels: Array,
  ...
  methods: {
    ...
    threeRender () {
      ...
      this.Graph.height(750).width(1200)
        ...
        .onNodeClick(node => {
          console.log(node)
          //设置#info h4样式的�?�色为�?�节点的颜色，文�?为�?�节点name
